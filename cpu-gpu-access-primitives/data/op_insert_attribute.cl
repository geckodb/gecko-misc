...
__kernel void op_insert_attribute(__global char* input_data, __global char* output_data, global int offsets[], int pos)
{
    const int g_id = get_global_id(0);

    update_customer(output_data, g_id, read_customer(input_data, g_id, pos, offsets), pos, offsets);
