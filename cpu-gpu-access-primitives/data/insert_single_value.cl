...
kernel void insert_single_value(global char* input_data, global char* output_data, global int offsets[], 
                                global int *num_of_attributes)
{
    const int g_id = get_global_id(1);
    const int attr_id = get_global_id(0);
    
    write_value(output_data, g_id, read_value(input_data, g_id, attr_id, offsets, num_of_attributes), attr_id, offsets, num_of_attributes);
}
