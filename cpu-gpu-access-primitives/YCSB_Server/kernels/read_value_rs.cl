...
kernel void select_single_value(global char* input_data, global int* indexes, global long* fields,
								global char* output_data, global int offsets[])
{
    const int g_id = get_global_id(1);
    const int attr_id = get_global_id(0);
    const int index = indexes[g_id];
    
    write_value(output_data, g_id, read_value(input_data, index, attr_id, offsets), attr_id, offsets);
}
